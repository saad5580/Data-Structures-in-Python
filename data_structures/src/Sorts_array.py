...
class Sorts:
    ...
    def radix_sort(a):
        ...
        """
        if len(a) > 0:
            max_digit = floor(log(max(a), 10) + 1)
            bucket = []
            for _ in range(10):
                bucket.append([])
            for num in range(max_digit):
                for value in a:
                    current_digit = value % ((10 ** num) * 10) // (10 ** num)
                    bucket[current_digit].append(value)
                index = 0
                for b in bucket:
                    while len(b) > 0:
                        a[index] = b.pop(0)
        ...
        while index < len(a):
            if index == 0:
                ...
                if a[index] < a[index - 1]:
                    # we are able to switch both values at the same time
                    # Because python will evaluate the left side all at once
                    # Then the right side
