...
class List:
    ...
    def min(self):
        ...
        count = 0
        current = self._front
        min_val = self._front._value
        while count < self._count:
            if current._value < min_val:
                min_val = current._value
            count += 1
            current = current._next
        return deepcopy(min_val)
    ...
    def _reverse_r_aux(self, previous, current):
        ...
        if current._next == None:
            ...
            current._next = previous
            previous = current
            self._reverse_r_aux(previous, n)
        return
    ...
    def is_identical_r(self, other):
        ...
        if other._count != self._count:
            ...
            if self._front._value != other._front._value:
                ...
                prev_front = other._front
                prev_front_self = self._front
                other._front = other._front._next
                self._front = self._front._next
                identical = self.is_identical_r(other)
                other._front = prev_front
                self._front = prev_front_self
        return identical
    ...
    def split_alt_r(self):
        ...
        odd = List()
        first = True
        self._split_alt_r_aux(even, odd, first)
        return even, odd

    def _split_alt_r_aux(self, target1, target2, even):
        """
        -------------------------------------------------------
        Split a list into two parts. even contains the even indexed
        elements, odd contains the odd indexed elements. At finish
        ...
        if self._count > 0:
            if even:
                ...
                target2._move_front(self)
            even = not even
            self._split_alt_r_aux(target1, target2, even)
        return
    ...
    def intersection_r(self, source1, source2):
        ...
        if source1._front != None and source2._front != None:
            index = 0
            self._intersection_r_aux(index, source1._front, source1, source2)
        return

    def _intersection_r_aux(self, index, current, source1, source2):
        """
        -------------------------------------------------------
        Update the current list with values that appear in both
        source1 and source2. Values do not repeat. source1 and
        ...
        if index < source1._count:
            *_, i = source2._linear_search(current._value)
            ...
            index += 1
            self._intersection_r_aux(index, current._next, source1, source2)
        return
    ...
    def union_r(self, source1, source2):
        ...
        if source1._front != None:
            index = 0
            select = 0
            self._union_r_aux(select, index, source1._front, source1, source2)
        else:
            if source2._front != None:
                index = 0
                select = 1
                self._union_r_aux(
                    select, index, source2._front, source1, source2)
        return

    def _union_r_aux(self, select, index, current, source1, source2):
        """
        -------------------------------------------------------
        Update the current list with all values that appear in
        source1 and source2. Values do not repeat. source1 and
        ...
        if select == 0:
            *_, i = self._linear_search(current._value)
            ...
            if index < source1._count:
                self._union_r_aux(
                    select, index, current._next, source1, source2)
            else:
                select = 1
                index = 0
                self._union_r_aux(
